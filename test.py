...
move_action1 = RunAction("MoveAction", lambda: chassis_system.move(v=-50, omega=50, vel_angle=math.radians(270), robot_theta=0))
...
complete_action= move_action1.withTimeout(1).andThen(move_action2.withTimeout(1)).andThen(move_action3.withTimeout(1)).andThen(move_action4.withTimeout(1)).andThen(final_action)
